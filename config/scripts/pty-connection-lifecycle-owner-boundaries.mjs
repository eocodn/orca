...
export function registerPtyConnectionLifecycleOwnerBoundaryTests(read) {
  ...
  it('routes hidden restore deferred retries through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-deferred-retry-controller.ts'
    )
  ...
  it('routes hidden restore replay baselines through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-replay-baseline-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionHiddenRestoreReplayBaselineController')
    expect(orchestrator).not.toContain('let hiddenOutputRestoreReplayingSnapshot:')
    expect(owner).toContain(
  ...
  it('routes hidden restore pending live queue state through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-pending-live-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionHiddenRestorePendingLiveController')
    expect(orchestrator).not.toContain('let hiddenOutputRestorePendingChunks:')
    expect(orchestrator).not.toContain('let hiddenOutputRestorePendingChars =')
    expect(orchestrator).not.toContain('let hiddenOutputRestorePendingOverflow =')
    expect(orchestrator).not.toContain('function getChunkDataAfterSnapshot(')
    expect(orchestrator).not.toContain('function drainPendingLiveChunksAfterSnapshot(')
    expect(orchestrator).not.toContain('function discardPendingLiveChunksSalvagingQueries(')
    expect(owner).toContain('export function createPtyConnectionHiddenRestorePendingLiveController')
    expect(owner.split(/\r?\n/).length).toBeLessThanOrEqual(300)
  })
  ...
  it('routes hidden restore task identity through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-task-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionHiddenRestoreTaskController')
    expect(orchestrator).not.toContain('let hiddenOutputRestoreInFlight:')
    expect(owner).toContain('export function createPtyConnectionHiddenRestoreTaskController')
    expect(owner.split(/\r?\n/).length).toBeLessThanOrEqual(300)
  })

  it('routes hidden restore snapshot scroll tickets through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-scroll-ticket-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionHiddenRestoreScrollTicketController')
    expect(orchestrator).not.toContain('let hiddenOutputSnapshotScrollRestore:')
    expect(owner).toContain(
  ...
  it('routes hidden restore snapshot replay through its concrete controller', () => {
    const orchestrator = read(
      'src/renderer/src/components/terminal-pane/pty-connection-session-orchestrator-runtime.ts'
    )
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-hidden-restore-snapshot-replay-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionHiddenRestoreSnapshotReplayController')
    expect(orchestrator).not.toContain('async function applyMainBufferSnapshot(')
    expect(owner).toContain(
  ...
  it('routes certified-dead restore recovery claims through its concrete controller', () => {
    ...
    const owner = read(
      'src/renderer/src/components/terminal-pane/pty-connection-certified-dead-restore-recovery-controller.ts'
    )

    expect(orchestrator).toContain('createPtyConnectionCertifiedDeadRestoreRecoveryController')
    expect(orchestrator).not.toContain('let certifiedDeadRestoreRecoveryRequested =')
    expect(owner).toContain(
