...
describe('Windows Tauri package delivery', () => {
  ...
  it('installs, observes, and uninstalls the package on the dedicated Windows 10 runner', () => {
    ...
    expect(acceptance).toContain("'control_status'")
    expect(acceptance).toContain('--endpoint-file')
    expect(acceptance).toContain('CommandLine')
    expect(acceptance).toContain('RedirectStandardInput = $true')
    expect(acceptance).toContain('UninstallString')
    expect(acceptance).toContain(".Trim().Trim('\"')")
    expect(acceptance).toContain('uninstallDeadline')
    expect(source).not.toContain('continue-on-error')
  })
})
