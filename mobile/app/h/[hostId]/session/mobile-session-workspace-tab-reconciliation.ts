...
import { acceptSessionSnapshot, applyClosedTabTombstones, confirmsMirroredTabSelection } from '../../../../src/session/session-tab-snapshot-gate'
import { getTerminalRecordsFromSessionTabs, mergeTerminalRecordsByCurrentOrder, mobileSessionTabsEqual, terminalRecordsEqual } from '../../../../src/session/mobile-terminal-records'
import { getActiveTabIdForHandle } from '../../../../src/session/mobile-session-route-helpers'
import type { SessionTabsApplyOutcome } from '../../../../src/session/mobile-session-tabs-stream-health'
import type { MobileSessionTab, SessionTabsResult } from './mobile-session-route-types'

type WorkspaceContext = Record<string, any>

export function useMobileSessionWorkspaceTabReconciliation(context: WorkspaceContext) {
  const { terminalDiagnosticsRef, appliedSnapshotMarkerRef, appliedSessionTabsRevisionRef, closedTabTombstonesRef,
    markdownDocsRef, sessionTabsRef, activeSessionTabIdRef, setSessionTabs, initialSessionAutoCreateRef,
    defaultTerminalHandlesToLiveInput, terminalsRef, setTerminals, lastKnownTerminalCountRef, setTerminalsLoaded,
    pendingActiveSessionTabIdRef, pendingActiveTerminalHandleRef, confirmsMirroredTabSelection: confirmSelection = confirmsMirroredTabSelection,
    activeSessionTabTypeRef, activeHandleRef, setActiveHandle, subscribeToTerminal, unsubscribeTerminal,
    initializedHandlesRef, setActiveSessionTabId } = context
