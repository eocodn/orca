import * as loadDependencies from './persistence-store-repository-load-dependencies'
const { app, safeStorage, readFileSync, writeFileSync, mkdirSync, existsSync, renameSync, unlinkSync, copyFileSync, statSync, realpathSync, rename, mkdir, rm, copyFile, open, renameDurableSync, writeFileDurableSync, join, dirname, isAbsolute, resolve, sep, homedir, createHash, randomUUID, latestAutomationOccurrenceAtOrBefore, nextAutomationOccurrenceAfter, getAutomationLegacyRepoId, normalizeAutomationPrecheck, deriveGlobalWindowsRuntimeDefaultFromLegacySettings, normalizeProjectRuntimePreference, projectHostSetupProjectionFromRepos, isPluginPanelTabKey, areTaskSourceContextsEqual, buildTaskSourceContextFromRepo, buildWorkspaceRunContext, normalizeStoredTaskSourceContext, areWorkspaceLinkedItemsEqual, normalizeWorkspaceLinkedItem, isWorkspaceLinkedItemSourceContextMatch, MOBILE_PAIRING_USERDATA_FILES, normalizePersistedMobileClientTabSelections, sanitizeWorkspaceSessionTerminalRetirements, removeRepoFromHostWorkspaceSessions, removeRepoFromWorkspaceSession, hardenExistingSecureFile, LEGACY_DEFAULT_SSH_RELAY_GRACE_PERIOD_SECONDS, isFolderRepo, getRepoExecutionHostId, parseExecutionHostId, LOCAL_EXECUTION_HOST_ID, normalizeExecutionHostOrder, normalizeExecutionHostId, normalizeVisibleExecutionHostIds, toSshExecutionHostId, getDefaultPersistedState, getDefaultNotificationSettings, getDefaultOnboardingState, getDefaultVoiceSettings, getDefaultUIState, getDefaultRepoHookSettings, getDefaultWorkspaceSession, getWorktreeCardModeProperties, isDefaultedCompactWorktreeCardProperties, normalizeAgentActivityDisplayMode, normalizeWorktreeCardProperties, ONBOARDING_FLOW_VERSION, ONBOARDING_FINAL_STEP, parseWorkspaceSession, normalizeUsagePercentageDisplay, normalizeStatusBarUsageMode, isExistingPersistedProfile, resolveUsagePercentageDisplayChangeNoticeDismissed, normalizePRBotAuthorOverrides, toRelaySshPtyId, migrateUiHostScopeSshTargetId, migrateWorkspaceSessionSshTargetId, isWslUncPath, isTerminalLeafId, makePaneKey, parseLegacyNumericPaneKey, parsePaneKey, setMigrationUnsupportedPty, setMigrationUnsupportedPtyPersistenceListener, agentHookServer, pruneLocalTerminalScrollbackBuffers, backfillAutomationRunNumbers, nextAutomationRunNumber, pruneAutomationRuns, pruneWorkspaceSessionBrowserHistory, FOLDER_WORKSPACE_INSTANCE_SEPARATOR, getRepoIdFromWorktreeId, getWorktreePathBasenameFromId, isPathInsideOrEqual, isWindowsAbsolutePathLike, normalizeRuntimePathForComparison, normalizeTerminalQuickCommands, normalizeTaskProviderSettings, normalizeAutoRenameBranchFromWorkDefaultOn, normalizeOpenInApplications, normalizeTerminalShortcutPolicy, normalizeSourceControlGroupOrder, normalizeAppIconId, normalizeTerminalCustomThemes, legacyTerminalScrollbackBytesToRows, normalizeDesktopTerminalScrollbackRows, compareFeatureInteractionUsageBuckets, getFeatureInteractionCategory, getFeatureInteractionUsageBucket, normalizeFeatureInteractions, normalizeFeatureInteractionTelemetryBuckets, normalizeContextualTourIds, normalizeFeatureTipIds, parseCodexResetCreditAttemptLedger, normalizeManualRepoOrder, DEFAULT_WORKSPACE_STATUS_ID, clampWorkspaceBoardColumnWidth, clampWorkspaceBoardOpacity, normalizePersistedWorkspaceStatuses, normalizeWorkspaceStatuses, clampMarkdownTocPanelWidth, clampCombinedDiffFileTreeWidth, isLegacyRepoForExternalWorktreeVisibility, sanitizeRepoIcon, normalizeRepoBadgeColor, clearMissingProjectGroupMemberships, createProjectGroup, getNextProjectGroupOrder, getProjectGroupSubtreeIds, normalizeProjectGroupName, normalizeProjectGroups, createNestedProjectGroupResolver, mergeLegacyCommitMessageAiIntoSourceControlAi, normalizeRepoSourceControlAiOverrides, normalizeSourceControlAiSettings, projectSourceControlAiToLegacyCommitMessageAi, sourceControlAiSettingsFromLegacy, DEFAULT_SOURCE_CONTROL_ACTION_COMMAND_TEMPLATES, SOURCE_CONTROL_TEXT_ACTION_IDS, normalizeDisabledTuiAgents, DEFAULT_TUI_AGENT_ARGS, DEFAULT_TUI_AGENT_ENV, hasUnsupportedTuiAgentArgs, normalizeTuiAgentArgsRecord, normalizeTuiAgentEnvRecord, normalizeTerminalCursorStyleDefault, normalizeOsc52ClipboardDefaultOn, osc52ClipboardDefaultOnOverridesPersistedOff, normalizeTerminalLineHeight, normalizeUiLanguage, normalizeBrowserPageZoomLevel, persistedUIValuesEqual, ActiveViewPreference, normalizeFolderWorkspaceName, normalizeFolderWorkspaceOperationId, normalizeFolderWorkspaces, folderWorkspaceKey, isWorkspaceKey, parseWorkspaceKey, worktreeWorkspaceKey, collectTerminalScrollbackSnapshotRefs, deleteTerminalScrollbackSnapshotSync, getProfileTerminalScrollbackSnapshotRoot, migrateWorkspaceSessionTerminalScrollbackSnapshots, readTerminalScrollbackSnapshotSync, track, getCohortAtEmit, isStartupDiagnosticsEnabled, logStartupDiagnostic, cloneLayoutNode, cloneLayoutWithLeafIds, collectLayoutLeafCounts, collectLayoutLeafIdsInOrder, findWorktreeIdForTab, firstLayoutLeafId, leafRecordEquivalent, layoutContainsLeafId, preserveMissingLeafRecordEntries, remapLeafRecordForPersistence, removeWorkspaceSessionOwner, removeWorkspaceSessionOwners, inferFolderScopeConnectionIdForMigration, backfillFolderScopeConnectionIds, deleteRemovedTerminalScrollbackSnapshots, getDefaultWorktreeMeta, StoreFoundation } = loadDependencies
    ...
    try {
      if (fileExistedOnLoad) {
        ...
        const migratedFloatingTerminalEnabled = floatingTerminalDefaultedForAllUsers
          ...
          : true
        const migratedOsc52Clipboard = loadDependencies.normalizeOsc52ClipboardDefaultOn(parsed.settings)
        ...
        const migratedAutoRenameBranchFromWork = loadDependencies.normalizeAutoRenameBranchFromWorkDefaultOn(
          parsed.settings
        )
        const migratedTerminalCursorStyle = loadDependencies.normalizeTerminalCursorStyleDefault(parsed.settings)
        ...
        if (
          ...
          loadDependencies.hasUnsupportedTuiAgentArgs('opencode', parsed.settings?.agentDefaultArgs?.opencode) ||
        ...
        const migratedWindowsRuntimeDefault =
          parsed.settings?.localWindowsRuntimeDefault === undefined
            ? loadDependencies.deriveGlobalWindowsRuntimeDefaultFromLegacySettings(parsed.settings).defaultRuntime
        ...
        result = {
          ...defaults,
          ...parsed,
          featureInteractionTelemetryBuckets: loadDependencies.normalizeFeatureInteractionTelemetryBuckets(
          ...
          mobileClientTabSelectionsByDeviceId: loadDependencies.normalizePersistedMobileClientTabSelections(
          ...
          settings: {
            ...
            openInApplications: loadDependencies.normalizeOpenInApplications(parsed.settings?.openInApplications, {
              seedDefaults: true
            }),
          ...
          automationRuns: (() => {
            ...
            const runs = loadDependencies.pruneAutomationRuns(loadDependencies.backfillAutomationRunNumbers(parsed.automationRuns))
