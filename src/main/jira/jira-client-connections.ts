...
import { requestWithCredentials, jiraRequest } from './jira-client-request'
function getClients(selection?: JiraSiteSelection | null): JiraClientForSite[] {
...
function getStatus(): JiraConnectionStatus {
  const file = getSiteFile()
...
function selectSite(siteId: JiraSiteSelection): JiraConnectionStatus {
  const file = getSiteFile()
...
function clearToken(siteId: string): void {
  deleteToken(siteId)
