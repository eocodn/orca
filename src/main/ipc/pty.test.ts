...
import { hasLivePtys, markPtySpawned } from '../pty/pty-lifecycle-state'
import * as livePtyState from '../pty/pty-lifecycle-state'
import {
...
async function prepareCodexSessionResume(args: {
  [key: string]: unknown
...
> {
  ...
  const homePath = args.trustedCodexHomes.find(
    (home) => transcriptPath.startsWith(`${home}/`) && transcriptPath.includes('/sessions/')
  )
  if (!homePath) {
    return { outcome: 'fresh', claimedCodexProvenance: transcriptPath.includes('/sessions/') }
  }
  const resolvedHome = args.resolveVerifiedResumeHome
    ? await args.resolveVerifiedResumeHome({ homePath, transcriptPath })
    : homePath
  return { outcome: 'resume', codexHomePath: resolvedHome }
}
...
describe('registerPtyHandlers', () => {
  ...
  it('attempts cleanup when a fresh provider spawn has no incarnation identity', async () => {
    const ptyId = 'pty-fresh-without-incarnation'
    ...
    await expect(
      handlers.get('pty:spawn')!(null, {
        ...
        cwd: '/tmp/worktree',
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-fresh-without-incarnation',
  ...
  it('rejects a same-id spawn that was in flight when failed cleanup became authoritative', async () => {
    const ptyId = 'pty-failed-spawn-replacement-race'
    const firstSpawn = makeDeferred<{ id: string; incarnationId: string }>()
    const replacementSpawn = makeDeferred<{ id: string; incarnationId: string }>()
    const shutdown = makeDeferred<void>()
    let spawnCall = 0
    const provider = createAgentClaimProvider({
      spawn: vi.fn(() => (spawnCall++ === 0 ? firstSpawn.promise : replacementSpawn.promise)),
    ...
    const replacement = controller!.spawn({
      ...spawnArgs,
      tabId: 'replacement',
    ...
    shutdown.resolve()
    await expect(replacement).rejects.toThrow('pty_cleanup_pending')
    await expect(first).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    expect(persistPtyBinding).toHaveBeenCalledOnce()
  })

  it('does not let an identity-less async kill clear a replacement', async () => {
    ...
    setLocalPtyProvider(createAgentClaimProvider({ shutdown }) as never)
    handlers.clear()
    registerPtyHandlers(mainWindow as never, runtime as never)
    ...
    expect(controller.kill('legacy-pty')).toBe(true)
    await Promise.resolve()
    restorePtyIncarnation('legacy-pty', 'replacement-incarnation')
    shutdownDeferred.resolve()
    await vi.waitFor(() => expect(shutdown).toHaveBeenCalledOnce())
    await Promise.resolve()

    expect(runtime.onPtyExit).not.toHaveBeenCalled()
    expect(isCurrentPtyExit({ id: 'legacy-pty', incarnationId: 'replacement-incarnation' })).toBe(
      true
    )
    clearProviderPtyState('legacy-pty')
  })

  it('fails closed for an identity-bearing exit after its lifecycle state is cleared', () => {
    const ptyId = 'pty-cleared-lifecycle-exit'
    getOrCreatePtyStateToken(ptyId)
    clearProviderPtyState(ptyId)

    expect(isCurrentPtyExit({ id: ptyId, incarnationId: 'delayed-old-exit' })).toBe(false)
  })

  it('rejects an identity-bearing exit when only an identity-less lifecycle token remains', () => {
    const ptyId = 'pty-identityless-current-lifecycle'
    getOrCreatePtyStateToken(ptyId)

    expect(isCurrentPtyExit({ id: ptyId, incarnationId: 'stale-exact-exit' })).toBe(false)
  ...
  it('does not let an identity-less provider exit clear an id-reused PTY', async () => {
    const ptyId = 'legacy-provider-pty'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const provider = createAgentClaimProvider({
  ...
  it('accepts an identity-less exit after authoritative absence despite a restored current identity', async () => {
    const ptyId = 'identity-less-restored-owner-pty'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const provider = createAgentClaimProvider({
    ...
    await handlers.get('pty:spawn')!(null, { cols: 80, rows: 24 })
    restorePtyIncarnation(ptyId, 'restored-owner-incarnation')

    exitHandler?.({ id: ptyId, code: 19 })
    await vi.waitFor(() =>
      expect(mainWindow.webContents.send).toHaveBeenCalledWith('pty:exit', {
        id: ptyId,
        code: 19
  ...
  it('does not select a replacement after local provider startup completes', async () => {
    ...
    const shutdown = vi.fn(async () => undefined)
    const provider = createAgentClaimProvider({ shutdown })
    const runtime = {
      setPtyController: vi.fn(),
      onPtyExit: vi.fn()
    }
    setLocalPtyProvider(provider as never)
    restorePtyIncarnation('startup-race-pty', 'incarnation-a')
    handlers.clear()
    registerPtyHandlers(
      mainWindow as never,
      runtime as never,
      ...
      undefined,
      undefined,
      { awaitLocalPtyProviderStartup: () => startupDeferred.promise }
    )
    const controller = runtime.setPtyController.mock.calls[0]?.[0] as {
      kill: (ptyId: string) => boolean
    }

    expect(controller.kill('startup-race-pty')).toBe(true)
    restorePtyIncarnation('startup-race-pty', 'incarnation-b')
    ...
    expect(runtime.onPtyExit).not.toHaveBeenCalled()
    clearProviderPtyState('startup-race-pty')
  })

  it('does not trust a stale hasPty false over live provider inventory', async () => {
    const ptyId = 'pty-stale-has-pty-false'
    const incarnationId = 'inc-stale-has-pty-false'
    const listProcesses = vi.fn(async () => [
      { id: ptyId, incarnationId, cwd: '/tmp/worktree', title: 'shell' }
    ])
    const provider = createAgentClaimProvider({
      spawn: vi.fn(async () => ({ id: ptyId, incarnationId })),
    ...
    await expect(
      handlers.get('pty:spawn')!(null, {
        ...
        cwd: '/tmp/worktree',
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-stale-has-pty-false',
        leafId: '88888888-8888-4888-8888-888888888888'
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)

    expect(listProcesses).toHaveBeenCalled()
    expect(getPendingPtyCleanupIncarnation(ptyId)).toBe(incarnationId)
    listProcesses.mockResolvedValue([])
    await expect(reconcilePendingPtyCleanup(provider as never, ptyId)).resolves.toBe(true)
  })

  it('retains id-only cleanup authority until provider absence is proven', async () => {
    const ptyId = 'pty-fresh-without-incarnation-retry'
    const shutdown = vi.fn().mockRejectedValue(new Error('shutdown response lost'))
    ...
    await expect(
      handlers.get('pty:spawn')!(null, {
        ...
        cwd: '/tmp/worktree',
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-fresh-without-incarnation-retry',
        leafId: '77777777-7777-4777-8777-777777777777'
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('re-arms an id-only cleanup retry after an identity-bearing replacement exits', async () => {
    const ptyId = 'pty-id-only-replacement-exit-retry'
    const listProcesses = vi
      .fn()
      .mockRejectedValueOnce(new Error('provider unavailable'))
      .mockResolvedValueOnce([{ id: ptyId, incarnationId: 'replacement-incarnation' }])
      .mockResolvedValue([])
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    ...
    await expect(
      handlers.get('pty:spawn')!(null, {
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-id-only-replacement-exit-retry',
    ...
    restorePtyIncarnation(ptyId, 'replacement-incarnation')
    exitHandler?.({ id: ptyId, code: 0, incarnationId: 'replacement-incarnation' })
    await vi.waitFor(() => expect(hasPendingPtyCleanupWithoutIncarnation(ptyId)).toBe(false))
  ...
  it('automatically retries cleanup after a transient inventory failure', async () => {
    ...
    await expect(
      handlers.get('pty:spawn')!(null, {
        ...
        cwd: '/tmp/worktree',
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-cleanup-inventory-retry',
        leafId: '77777777-7777-4777-8777-777777777777'
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('does not let stale identity-less cleanup restore a same-id replacement', async () => {
    const ptyId = 'pty-cleanup-stale-token'
    const inventory = makeDeferred<{ id: string; incarnationId?: string }[]>()
    const provider = createAgentClaimProvider({
      spawn: vi.fn(async () => ({ id: ptyId })),
      shutdown: vi.fn(async () => undefined),
    ...
    setLocalPtyProvider(provider as never)
    restorePtyIncarnation(ptyId, 'incarnation-before-failure')
    const runtime = {
      setPtyController: vi.fn(),
      createPreAllocatedTerminalHandle: vi.fn(() => null),
      preAllocateHandleForPty: vi.fn(),
    ...
    const spawn = handlers.get('pty:spawn')!(null, {
      ...
      cwd: '/tmp/worktree',
      worktreeId: 'repo::/tmp/worktree',
      tabId: 'tab-cleanup-stale-token',
      leafId: '88888888-8888-4888-8888-888888888888'
    })
    await vi.waitFor(() => expect(provider.listProcesses).toHaveBeenCalledOnce())
    restorePtyIncarnation(ptyId, 'incarnation-replacement')
    inventory.resolve([])

    await expect(spawn).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    expect(isCurrentPtyExit({ id: ptyId, incarnationId: 'incarnation-replacement' })).toBe(true)
    expect(runtime.onPtyExit).not.toHaveBeenCalled()
    clearProviderPtyState(ptyId)
  })

  it('keeps an unchanged owner lifecycle token stable during kill', async () => {
    const ptyId = 'pty-owner-kill-token'
    const incarnationId = 'incarnation-owner-kill-token'
    const shutdown = makeDeferred<void>()
    const owner: AgentSessionOwnerBinding = {
      claim: recoveredAgentClaim,
      generation: 'generation-owner-kill-token',
      phase: 'live',
      ptyId,
      surface: recoveredAgentSurface
    }
    ...
    ).resolves.toMatchObject({ id: ptyId, agentSessionEnsure: { disposition: 'adopted' } })
    shutdown.resolve()

    await vi.waitFor(() => expect(runtime.onPtyExit).toHaveBeenCalledWith(ptyId, -1, incarnationId))
    clearProviderPtyState(ptyId)
  })
  ...
  async function spawnAndGetEnv(
    ...
    // Why: daemon-backed callers can pass the launch command through to host env preparation.
    command?: string,
  ...
  describe('spawn environment', () => {
    ...
    it('tracks local PTY launches until the PTY is killed', async () => {
      ...
      expect(hasLivePtys()).toBe(true)

      await handlers.get('pty:kill')!(null, { id: spawnResult.id })

      expect(hasLivePtys()).toBe(false)
    })

    it('clears PTY tracking from shared provider teardown', () => {
      markPtySpawned('ssh-pty')
      expect(hasLivePtys()).toBe(true)

      clearProviderPtyState('ssh-pty')
    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      async function daemonSpawnAndGetOptions(
        ...
        processEnvOverrides?: Record<string, string | undefined>,
        // Why: daemon spawn tests exercise WSL launch metadata and command forwarding.
        spawnArgs?: {
          cwd?: string
          worktreeId?: string
          shellOverride?: string
      ...
      it('forwards ORCA_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        ...
        try {
          ...
          // Keep provider cleanup scoped to the test's registered SSH target.
  ...
  it('does not let an earlier async kill completion clear a replacement incarnation', async () => {
    ...
    restorePtyIncarnation('local-pty', 'incarnation-a')
    handlers.clear()
    registerPtyHandlers(mainWindow as never, runtime as never)
    const controller = runtime.setPtyController.mock.calls[0]?.[0] as {
      kill: (ptyId: string) => boolean
    ...
    expect(controller.kill('local-pty')).toBe(true)
    await Promise.resolve()
    restorePtyIncarnation('local-pty', 'incarnation-b')
    shutdownDeferred.resolve()
    await vi.waitFor(() => expect(shutdown).toHaveBeenCalledOnce())
    await Promise.resolve()
  ...
  it('restores the prior publication after quarantined same-id cleanup is proven', async () => {
    ...
    await vi.waitFor(() =>
      expect(hasPendingPtyCleanupExact(sessionId, 'inc-cleanup-2')).toBe(false)
    )
    await expect(
      (controller as unknown as RuntimeSpawnController).spawn({ ...args, cols: 100, rows: 40 })
  ...
  it('does not publish failed incarnation exit while a replacement is staged', async () => {
    ...
    await expect(replacementSpawn).rejects.toThrow('pty_cleanup_pending')
  ...
  it('reconciles through a generation-less current SSH provider replacement', async () => {
    const connectionId = 'ssh-generationless-provider-replacement'
    const sessionId = 'pty-ssh-generationless-provider-replacement'
    const appPtyId = `ssh:${connectionId}@@${sessionId}`
    const oldProvider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: appPtyId, incarnationId: 'inc-generationless-old' }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses: vi.fn().mockRejectedValue(new Error('old relay disposed')),
      authoritativeOwnerListings: false
    })
    const newListProcesses = vi.fn().mockResolvedValue([])
    const newProvider = createAgentClaimProvider({
      listProcesses: newListProcesses,
      authoritativeOwnerListings: false
    })
    ...
    const spawn = controller.spawn({
      ...
      connectionId,
      sessionId,
      worktreeId: 'wt-generationless-provider-replacement',
      tabId: 'tab-generationless-provider-replacement',
    ...
    await vi.waitFor(() => expect(newListProcesses).toHaveBeenCalled())
    expect(getPendingPtyCleanupIncarnation(appPtyId)).toBeUndefined()
    unregisterSshPtyProvider(connectionId)
  })
  ...
  it('retains exact cleanup authority when its finalizer fails once', async () => {
    ...
    expect(getPendingPtyCleanupIncarnation(ptyId)).toBe(incarnationId)
    runtime.onPtyExit.mockImplementationOnce(() => {
      throw new Error('runtime finalizer failed')
    })
    expect(() => finalizePendingPtyCleanupIfExact({ id: ptyId, incarnationId })).toThrow(
      'runtime finalizer failed'
    )
    expect(getPendingPtyCleanupIncarnation(ptyId)).toBe(incarnationId)

    expect(finalizePendingPtyCleanupIfExact({ id: ptyId, incarnationId })).toBe(true)
  })

  it('keeps an exact tombstone retryable when a provider exit finalizer fails', async () => {
    const ptyId = 'pty-provider-exit-finalizer-failure'
    const incarnationId = 'inc-provider-exit-finalizer-failure'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi
      .fn()
      .mockResolvedValue([{ id: ptyId, incarnationId, cwd: '/tmp', title: 'live' }])
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses,
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-provider-exit-finalizer-failure',
        leafId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    await vi.waitFor(() => expect(listProcesses).toHaveBeenCalled())
    expect(hasPendingPtyCleanupExact(ptyId, incarnationId)).toBe(true)

    runtime.onPtyExit.mockImplementationOnce(() => {
      throw new Error('runtime finalizer failed')
    })
    expect(() => exitHandler?.({ id: ptyId, code: 0, incarnationId })).not.toThrow()
    expect(hasPendingPtyCleanupExact(ptyId, incarnationId)).toBe(true)

    runtime.onPtyExit.mockImplementation(() => undefined)
    exitHandler?.({ id: ptyId, code: 0, incarnationId })
    await vi.waitFor(() => expect(hasPendingPtyCleanupExact(ptyId, incarnationId)).toBe(false))
  })

  it('does not finalize an exact tombstone when inventory proves a replacement incarnation', async () => {
    const ptyId = 'pty-exact-superseded-inventory'
    const incarnationId = 'inc-exact-superseded'
    const inventory = makeDeferred<{ id: string; incarnationId?: string }[]>()
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-exact-superseded-inventory',
        leafId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    ...
    inventory.resolve([{ id: ptyId, incarnationId: 'replacement-incarnation' }])
    await vi.waitFor(() => expect(hasPendingPtyCleanupExact(ptyId, incarnationId)).toBe(false))

    expect(getPtyStateToken(ptyId)).toBeUndefined()
    expect(runtime.onPtyExit).not.toHaveBeenCalled()
    mainWindow.webContents.send.mockClear()
    exitHandler?.({ id: ptyId, code: 0, incarnationId })
    await Promise.resolve()
  ...
  it('does not finalize an exact tombstone while identity-less inventory reports the id live', async () => {
    const ptyId = 'pty-exact-live-identityless-inventory'
    const incarnationId = 'inc-exact-live-identityless'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi.fn().mockResolvedValue([{ id: ptyId }])
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses,
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-exact-live-identityless-inventory',
        leafId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('ignores an exact exit delivered by a provider after local provider replacement', async () => {
    const ptyId = 'pty-stale-provider-exit-after-rebind'
    const incarnationId = 'inc-stale-provider-exit-after-rebind'
    const oldInventory = makeDeferred<{ id: string; incarnationId?: string }[]>()
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const oldProvider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('old provider shutdown response lost')),
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-stale-provider-exit-after-rebind',
        leafId: 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    ...
    const replacementProvider = createAgentClaimProvider({
      listProcesses: vi.fn().mockResolvedValue([{ id: ptyId, incarnationId }]),
      authoritativeOwnerListings: false
    })
    setLocalPtyProvider(replacementProvider as never)
    expect(replacementProvider.listProcesses).not.toHaveBeenCalled()
  ...
  it('ignores stale data and transient callbacks from a replaced provider', () => {
    type DataHandler = (payload: { id: string; incarnationId: string; data: string }) => void
    type BackgroundHandler = (payload: {
      id: string
      kind: 'dataGap'
      droppedChars: number
    }) => void
    type WriteUnavailableHandler = (payload: { id: string }) => void
    let oldData: DataHandler | undefined
    let oldBackground: BackgroundHandler | undefined
    let oldWriteUnavailable: WriteUnavailableHandler | undefined
    const createProvider = () => {
      const base = createAgentClaimProvider({})
      return {
        ...base,
        ...
        onBackgroundStreamEvent: vi.fn((handler: BackgroundHandler) => {
          if (!oldBackground) {
            oldBackground = handler
          }
          return () => {}
        }),
        onWriteUnavailable: vi.fn((handler: WriteUnavailableHandler) => {
          if (!oldWriteUnavailable) {
            oldWriteUnavailable = handler
          }
          return () => {}
        })
      }
    ...
    const runtime = {
      ...
      setPtyTransientFactDelegation: vi.fn(),
      emitDaemonPtyTransientFact: vi.fn()
    }
    setLocalPtyProvider(oldProvider as never)
    registerPtyHandlers(mainWindow as never, runtime as never)
    const oldBackgroundHandler = oldBackground
    const oldWriteUnavailableHandler = oldWriteUnavailable
    setLocalPtyProvider(newProvider as never)
    rebindLocalProviderListeners()
    mainWindow.webContents.send.mockClear()

    oldData?.({ id: 'stale-provider-callback', incarnationId: 'old', data: 'stale' })
    oldBackgroundHandler?.({ id: 'stale-provider-callback', kind: 'dataGap', droppedChars: 3 })
    oldWriteUnavailableHandler?.({ id: 'stale-provider-callback' })

    expect(runtime.onPtyData).not.toHaveBeenCalled()
    expect(runtime.notePtyDataGap).not.toHaveBeenCalled()
    expect(runtime.setPtyTransientFactDelegation).not.toHaveBeenCalled()
    expect(mainWindow.webContents.send).not.toHaveBeenCalled()
  })

  it('does not finalize an id-less tombstone when inventory proves a replacement is live', async () => {
    const ptyId = 'pty-idless-live-replacement'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-idless-live-replacement',
        leafId: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('retries a pending cleanup when inventory still reports the same PTY live', async () => {
    vi.useFakeTimers()
    try {
      const ptyId = 'pty-live-cleanup-retry'
      const incarnationId = 'inc-live-cleanup-retry'
      const listProcesses = vi
        .fn()
        .mockResolvedValue([{ id: ptyId, incarnationId, cwd: '/tmp', title: 'live' }])
      const provider = createAgentClaimProvider({
      ...
      expect(hasPendingPtyCleanupExact(ptyId, incarnationId)).toBe(false)
    } finally {
      vi.clearAllTimers()
      vi.useRealTimers()
    }
  ...
  it('rechecks provider inventory before finalizing a delayed exact exit', async () => {
    const ptyId = 'pty-delayed-exact-replacement'
    const incarnationId = 'inc-delayed-exact'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi
      ...
      .mockResolvedValue([{ id: ptyId, incarnationId: 'replacement-incarnation' }])
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses,
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-delayed-exact-replacement',
        leafId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('ignores a retained local exit when the replacement provider has no same-id PTY', async () => {
    let exitCb: ((info: { exitCode: number }) => void) | undefined
    const proc = {
      onData: vi.fn(() => makeDisposable()),
      onExit: vi.fn((cb: (info: { exitCode: number }) => void) => {
    ...
    registerPtyHandlers(mainWindow as never, runtime as never)
    const spawned = (await handlers.get('pty:spawn')!(null, { cols: 80, rows: 24 })) as {
      id: string
    }
    setLocalPtyProvider({ hasPty: vi.fn(() => false) } as never)
    ...
    expect(runtime.onPtyExit).not.toHaveBeenCalled()
    expect(mainWindow.webContents.send).not.toHaveBeenCalledWith('pty:exit', {
      id: spawned.id,
      code: 0
    })
  })

  it('does not consume a synthetic kill exit belonging to a replacement incarnation', async () => {
    const ptyId = 'synthetic-kill-replacement'
    const exitHandlers = new Set<
      (payload: { id: string; code: number; incarnationId?: string }) => void
    >()
    const provider = createAgentClaimProvider({
      spawn: vi.fn(async () => ({ id: ptyId, incarnationId: 'synthetic-old' })),
      shutdown: vi.fn(async () => undefined),
    ...
    restorePtyIncarnation(ptyId, 'synthetic-new')

    for (const handler of exitHandlers) {
      handler({ id: ptyId, code: 0, incarnationId: 'synthetic-new' })
    }
    await Promise.resolve()
    await Promise.resolve()

    expect(runtime.onPtyExit).toHaveBeenCalledWith(ptyId, 0, 'synthetic-new')
  })

  it('does not admit a stale inventory result after its SSH provider is replaced', async () => {
    const oldInventory = makeDeferred<{ id: string; cwd: string; title: string }[]>()
    const oldProvider = createAgentClaimProvider({
      listProcesses: vi.fn(() => oldInventory.promise)
    })
    const replacementProvider = createAgentClaimProvider({
    ...
    registerSshPtyProvider('ssh-stale-inventory', oldProvider as never)
    registerPtyHandlers(mainWindow as never)
  ...
  it('rechecks lifecycle state after delayed exact inventory before retaining a tombstone', async () => {
    const ptyId = 'pty-delayed-exact-lifecycle-recheck'
    const incarnationId = 'inc-delayed-exact-lifecycle'
    const inventory = makeDeferred<{ id: string; incarnationId?: string }[]>()
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi
      .fn()
      .mockResolvedValueOnce([{ id: ptyId, incarnationId, cwd: '/tmp', title: 'old' }])
      .mockImplementationOnce(() => inventory.promise)
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses,
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-delayed-exact-lifecycle-recheck',
        leafId: 'dddddddd-dddd-4ddd-8ddd-dddddddddddd',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('re-arms exact cleanup after a different incarnation exits', async () => {
    const ptyId = 'pty-exact-replacement-exit-retry'
    const oldIncarnation = 'inc-exact-old'
    const replacementIncarnation = 'inc-exact-replacement'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const listProcesses = vi
      ...
      .mockResolvedValue([])
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId: oldIncarnation }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
      listProcesses,
      onExit: vi.fn((handler: typeof exitHandler) => {
        exitHandler = handler
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-exact-replacement-exit-retry',
        leafId: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    ...
    restorePtyIncarnation(ptyId, replacementIncarnation)
    exitHandler?.({ id: ptyId, code: 0, incarnationId: replacementIncarnation })
    await vi.waitFor(() => expect(hasPendingPtyCleanupExact(ptyId, oldIncarnation)).toBe(false))
    expect(listProcesses).toHaveBeenCalledTimes(2)
  })

  it('does not publish a stale exact cleanup exit after its state token is replaced', async () => {
    const ptyId = 'pty-finalizer-stale-token'
    const incarnationId = 'inc-finalizer-stale-token'
    let exitHandler:
      | ((payload: { id: string; code: number; incarnationId?: string }) => void)
      | undefined
    const provider = createAgentClaimProvider({
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-finalizer-stale-token',
        leafId: '77777777-7777-4777-8777-777777777777',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('automatically retries a failed cleanup finalizer', async () => {
    const ptyId = 'pty-finalizer-automatic-retry'
    const incarnationId = 'inc-finalizer-automatic-retry'
    const provider = createAgentClaimProvider({
      spawn: vi.fn().mockResolvedValue({ id: ptyId, incarnationId }),
      shutdown: vi.fn().mockRejectedValue(new Error('shutdown response lost')),
    ...
    const runtime = {
      setPtyController: vi.fn(),
      registerPty: vi.fn(),
      onPtyExit: vi.fn().mockImplementationOnce(() => {
        throw new Error('runtime finalizer failed')
      })
    }
    const store = {
    ...
    await expect(
      controller.spawn({
        ...
        sessionId: ptyId,
        worktreeId: 'repo::/tmp/worktree',
        tabId: 'tab-finalizer-automatic-retry',
        leafId: '99999999-9999-4999-8999-999999999999',
        persistHostSessionBinding: true
      })
    ).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)

    await vi.waitFor(() => expect(runtime.onPtyExit).toHaveBeenCalledTimes(2))
    expect(getPendingPtyCleanupIncarnation(ptyId)).toBeUndefined()
  })

  it('settles a pane reservation when exact cleanup finalization throws', async () => {
    const ptyId = 'pty-finalizer-reservation-throw'
    const tabId = '11111111-1111-4111-8111-111111111111'
    const leafId = '22222222-2222-4222-8222-222222222222'
    const provider = createAgentClaimProvider({
      spawn: vi
        .fn()
        .mockResolvedValueOnce({ id: ptyId, incarnationId: 'inc-reservation-throw' })
        .mockResolvedValueOnce({ id: ptyId, incarnationId: 'inc-reservation-retry' }),
      shutdown: vi.fn().mockResolvedValue(undefined),
      listProcesses: vi.fn().mockResolvedValue([]),
      authoritativeOwnerListings: false
    })
    const persistPtyBinding = vi
      ...
      .mockReturnValue({ rollbackIfCurrent: vi.fn(() => true) })
    const runtime = {
      setPtyController: vi.fn(),
      registerPty: vi.fn(),
      quarantinePtyAfterPublicationFailure: vi.fn(),
      hasObservedExactPtyExit: vi.fn(() => false),
      onPtyExit: vi.fn().mockImplementationOnce(() => {
        throw new Error('runtime finalizer failed')
      })
    }
    ...
    await expect(controller.spawn(args)).rejects.toThrow(/ORCA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
    await expect(controller.spawn(args)).resolves.toMatchObject({
      id: ptyId,
      incarnationId: 'inc-reservation-retry'
    })
    expect(provider.spawn).toHaveBeenCalledTimes(2)
  })
  ...
  it('fences a late SSH provider unregister from removing the replacement generation', () => {
    const connectionId = 'ssh-late-provider-unregister'
    const original = { providerGeneration: 41 }
    const replacement = { providerGeneration: 42 }
    registerSshPtyProvider(connectionId, original as never)
    registerSshPtyProvider(connectionId, replacement as never)

    const unregister = unregisterSshPtyProvider as unknown as (
      connection: string,
      expectedProvider: unknown
    ) => void
    unregister(connectionId, original)

    expect(getSshPtyProvider(connectionId)).toBe(replacement)
    unregisterSshPtyProvider(connectionId)
  })
  ...
  it('does not let a retained local exit retire a live same-id replacement', async () => {
    let exitCb: ((info: { exitCode: number }) => void) | undefined
    const proc = {
      onData: vi.fn(() => makeDisposable()),
      onExit: vi.fn((cb: (info: { exitCode: number }) => void) => {
    ...
    const replacement = {
      hasPty: vi.fn((id: string) => id === spawned.id)
    }
  ...
  it('does not sweep local PTYs during a recovery reload', async () => {
    ...
    const markPtyExitedSpy = vi.spyOn(livePtyState, 'markPtyExited')
