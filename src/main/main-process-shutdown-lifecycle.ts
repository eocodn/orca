...
startupDeps.app.on('will-quit', (e) => {
  ...
  startupState.runtime?.getAgentBrowserBridge()?.destroyAllSessions()
  ...
  if (!startupState.daemonDisconnectDone) {
      ...
      ? startupState.runtimeRpc
          ...
          .then(() => {
            if (ownedRuntimeId) {
              ...
              startupDeps.clearRuntimeMetadataIfOwned(startupDeps.getCanonicalUserDataPath(), ownedPid, ownedRuntimeId)
            }
          })
          .catch((error) => {
    ...
    const daemonTeardown = startupDeps.isDevParentShutdownRequested() ? startupDeps.shutdownDaemon() : startupDeps.disconnectDaemon()
    ...
    startupDeps.settleTeardownWithinDeadline([
