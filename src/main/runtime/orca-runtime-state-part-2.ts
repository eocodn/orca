...
export class OrcaRuntimeStatePart2 extends OrcaRuntimeStatePart1 {
  ...
  protected nextSyntheticPtyIncarnation = 1
  ...
  protected rendererGraphLivenessBlockedPtys = new Set<string>()
  ...
  protected remoteTerminalViewSubscriberCounts = new Map<string, number>()
  // Preview windows consume the raw stream but deliberately leave terminal
  // query replies to main's headless emulator.
  protected rawTerminalViewSubscriberCounts = new Map<string, number>()
  ...
  protected currentDriver = new Map<string, DriverState>()
  protected mobileInputFloorClaims = new Map<
    string,
    {
      base: DriverState
      generation: number
  ...
  >()
  protected currentBrowserDriver = new Map<string, RuntimeBrowserDriverState>()
  ...
  protected remoteDesktopViewers = new Map<
    string,
    Map<string, { clientId: string; cols: number; rows: number; activity: number }>
  >()
  protected remoteDesktopOwners = new Map<string, string>()
  protected remoteDesktopActivity = 0
  protected remoteDesktopHostReclaimTargets = new Map<string, { cols: number; rows: number }>()
  ...
  protected remoteDesktopViewerRevisions = new Map<string, number>()
