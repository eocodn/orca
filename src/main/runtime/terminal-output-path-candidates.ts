/* eslint-disable no-control-regex -- PTY paths must stop at ANSI escape sequences. */
...
export function appendRecentPtyPathCandidates(
  ...
  if (extractedCandidates.length === 0) {
    // Pathless output is the hot path; reuse immutable history.
    return previous ?? []
  }
...
// Keep candidate extraction line-scoped so a pathological PTY line cannot trigger quadratic regex work.
...
function collectTerminalOutputLinePathCandidates(line: string, add: (value: string) => void): void {
  for (const match of line.matchAll(/file:\/\/([^/\s]*)(\/[^\s\u001b"'<>)]*)/gi)) {
