...
export type RelaySocketTransportState = { stdoutAlive: boolean; flushStdoutDrainWaiters: () => void }
export type RelaySocketOwnership = { ownsSocketPath: boolean; ownedSocketIdentity: SocketIdentity | null }
...
export async function runRelaySocketLifecycle(state: RelaySocketLifecycleState): Promise<void> {
  const { dispatcher, detached, graceTimeMs, sockPath, endpointCredential, launchVersion,
    ptyHandler, ptyConsumerSessionAdapter, ptySourcePublication, fsHandler, gitHandler,
    hookServer, transportState, socketOwnership, ownsCurrentSocketPath, cleanupOwnedSocket } = state
