...
export type AgentTerminalOwnershipStore = {
  getState: () => AppState
  subscribe: (listener: (state: AppState, previousState: AppState) => void) => () => void
}

export type AgentTerminalOwnership = {
  finalize: () => boolean
  release: () => void
}

type CreateAgentTerminalOwnershipArgs = {
  store: AgentTerminalOwnershipStore
  worktreeId: string
  tabId: string
  paneKey: string
  ptyId: string
...
function isOwnedTabIdentityCurrent(
  state: AppState,
  args: Omit<CreateAgentTerminalOwnershipArgs, 'store' | 'runtimeKind'>
): boolean {
...
export function createAgentTerminalOwnership(
  args: CreateAgentTerminalOwnershipArgs
): AgentTerminalOwnership {
  let consumed = false
  ...
  return {
    ...
    finalize: () => {
      ...
      try {
        ...
        console.error('[agent-background] Failed to close owned agent terminal:', error)
        return false
      }
      return true
    }
...
export function bindAgentTerminal(
  tab: TerminalTab,
  paneKey: string,
  ptyId: string,
  runtimeKind: 'local' | 'environment',
  title?: string
): AgentTerminalOwnership | null {
