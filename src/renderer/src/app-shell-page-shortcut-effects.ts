  ...
  useEffect(() => {
    ...
    const dispatchShortcutInput = (input: ShortcutDispatchInput): void => {
      ...
      if (matchShortcut('sidebar.search.toggle') && canRevealRightSidebar) {
        // With a folder selected in the explorer, Cmd/Ctrl+Shift+F means "Find in Folder" â seed the include pattern with it, not a text search.
      ...
      // Floating panel closed â its keydown handler is gone, so honor the maximize chord here by opening it pre-maximized (no-op while it's open).
      if (
        !floatingTerminalOpen &&
      ...
      // Skip editable surfaces so TipTap's Cmd+B bold works; this renderer-side fallback covers the blurâpress IPC race (docs/markdown-cmd-b-bold-design.md).
