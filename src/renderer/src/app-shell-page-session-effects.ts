...
export function useAppShellPageSessionEffects(context: Record<string, unknown>) {
  const { acknowledgedAgentsByPaneKey, actions, activeView, combinedDiffFileTreeWidth, filterRepoIds, groupBy, hideAutomationGeneratedWorkspaces, hideCliCreatedWorkspaces, hideDefaultBranchWorkspace, hideDetachedHeadWorkspaces, markdownTocPanelWidth, persistedUIReady, projectOrderBy, rightSidebarExplorerView, rightSidebarOpen, rightSidebarTab, rightSidebarWidth, settings, showDotfilesByWorktree, showSleepingWorkspaces, sidebarWidth, sortBy, workspaceSessionReady } = context as any
  useEffect(() => {
