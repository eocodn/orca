...
export function useAppShellPageStartupEffects(context: Record<string, unknown>) {
  ...
  // Why: useLayoutEffect fires before paint, so dispatching SYNC_FIT_PANES_EVENT reflows the terminal in the same frame as the width change â no wrongly-sized transient.
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    let cancelled = false
    // Why: declared outside the async block so cleanup can abort it â under StrictMode the first (unmounted) pass would otherwise keep spawning PTYs.
    ...
    // Why (issue #1158): track whether success-path reconnect started so the catch doesn't re-run it â re-entering on partially-mutated state would double-set ptyIds and drain pending* twice.
    ...
    void (async () => {
      ...
      try {
        ...
        // Why: browser session profiles are NOT started early â on a remote runtime the RPC may be unconnected and a failed fetch clears the list.
        ...
        if (!cancelled) {
          ...
          if (!reconnectStarted) {
            ...
            // Why (issue #1158): reconnect already started; re-running over its partially-mutated state would double-set ptyIds and drain pending* twice â force the flag, clear pending*.
            useAppStore.setState({
              workspaceSessionReady: true,
              pendingReconnectWorktreeIds: [],
              pendingReconnectTabByWorktree: {},
