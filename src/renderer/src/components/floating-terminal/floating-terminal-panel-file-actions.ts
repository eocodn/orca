...
export function useFloatingTerminalPanelFileActions(state: PanelState) {
  const {
    floatingFiles,
    closeFile,
    markFileDirty,
  ...
  const advanceEditorCloseQueue = useCallback(() => {
    if (saveDialogFileIdRef.current !== null) return
  ...
  useEffect(() => {
    ...
    if (saveDialogFileId === null) advanceEditorCloseQueue()
