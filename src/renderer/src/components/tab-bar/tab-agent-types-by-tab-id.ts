 ...
 * the active pane wins when available. Before layout hydration, the first matching pane preserves
...
export function createTabBarAgentProjectionSelector(
  ...
  let cachedResult: TabBarAgentProjections | null = null

  return (state) => {
    if (!state.agentStatusByPaneKey && !state.terminalLayoutsByTabId) {
      if (cachedResult) {
    ...
    if (cachedResult?.tabAgentTypesByTabId === cachedAgentTypesByTabId) {
      return cachedResult
    }
    cachedResult = { tabAgentTypesByTabId: cachedAgentTypesByTabId }
    return cachedResult
  }
}
