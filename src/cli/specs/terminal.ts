...
export const TERMINAL_COMMAND_SPECS: CommandSpec[] = [
  ...
  {
    path: ['terminal', 'inspect'],
    summary: 'Inspect authoritative terminal lifecycle and history state',
    usage: 'orca terminal inspect [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal']
  },
  {
    path: ['terminal', 'resize'],
    summary: 'Resize a terminal with authoritative provider readback',
    usage:
      'orca terminal resize [--terminal <handle>] --incarnation <id> --cols <n> --rows <n> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'incarnation', 'cols', 'rows'],
    notes: [
      'Use processIncarnation from terminal inspect; stale incarnations are rejected before mutation.',
      'Success requires authoritative provider readback of the exact requested dimensions.'
    ]
  },
  {
    path: ['terminal', 'read'],
